...
export const Hero = ({ signedIn }: HeroProps) => {
  const [showLogoAnim, setShowLogoAnim] = useState(true);
  const [showSignup, setShowSignup] = useState(false);
  const scrollToCollection = () => {
    document.getElementById("collection")?.scrollIntoView({ behavior: "smooth" });
  };

  // When logo animation finishes, show signup popup if not signed in
  const handleLogoFinish = () => {
    setShowLogoAnim(false);
    if (!signedIn) setShowSignup(true);
  };
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      {showSignup && (
        <SignupLoginPopup open={showSignup} onClose={handleSignupClose} onSubmit={handleSignupSubmit} />
      )}

      {/* Main Content (hidden while logo animates) */}
      {!showLogoAnim && (
        <>
